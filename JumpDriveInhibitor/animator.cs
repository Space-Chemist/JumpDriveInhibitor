        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                foreach (var _beacon in beaconlist)
                {
                    ...
                    entity.SetEmissiveParts("Emissive", Color.Red, 1000f);
                    subpart.SetEmissiveParts("Emissive", Color.Red, 1000f);*/
                    
                    MyEntitySubpart subpart2;
                    if (subpart.TryGetSubpart("Ring", out subpart2))
                    {
                        RotationTime += 1;
                        var rotationMatrix = Matrix.CreateRotationY((0.01f * bp/10) * RotationTime);
